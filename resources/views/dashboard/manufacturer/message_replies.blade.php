...
<div class="col-md-10">
    <div class="row col-md-12 text-center " style="background-color: #AF2E1D !important; padding: 10px; text-align: center !important; color:white ">
        <h1 class="text-center m-auto pt-3 pb-3 text-capitalize"><b>{{ Auth::guard('manufacturer')->user()->full_name}}</b></h1>
    </div>


    @foreach($replies as $reply)
        @if($reply->replyer === 'manufacturer')
    ...
    <form action="{{ url('manufacturer/reply/' . $message_id) }}" method="post" class="row col-md-12 text-center mb-5" style="background-color: #fff !important; padding: 10px; text-align: center !important; color:white; border-top: solid #ccc 1px; ">
    @csrf
...
@push('scripts')
    <script>
        FormHandler('issueForm', {
            requestType: 'POST',
            requestUrl: '/manufacturer/contact-admin',
            cb: response => {
                if(response.success){
                    return swal({
                        title: 'Success!',
