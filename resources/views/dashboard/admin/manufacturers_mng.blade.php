                      ...
                      <table id="chat_table" class="display" style="width:100%" class="table table-responsive" style="width: 100%; cellspacing: 0;">
                          ...
                          <tbody>
                            @foreach($chats as $chat)
                              <tr>
                                  <td>{{ $chat->customer_full_name }}</td>
                                  <td>{{ $chat->customer_email }}</td>
                                  <td>{{ $chat->customer_phone_number }}</td>
                                  <td>{{ $chat->chat_topic }}</td>
                                  <td>{{ $chat->chat_replies->last()->message }}</td>
                                  <td>{{ $chat->manufacturer->email }}</td>
                                  <td>{{ $chat->manufacturer->company_name }}</td>
                                  <td>
                                    <a href="{{ url('/admin/manufacturer-replies', $chat->id) }}" target="_blank" style="background-color: #AF2E1D; color: white; border-radius: 20px; border: 1px solid white; display: block; padding: 10px;">View Replies</a></th>
                                  </td>
                                  <td>
                                    ...
                                    @if($chat->has_request())
                                      ...
                                      data-request_id="{{$chat->has_request()->id}}"
                                      data-supply_request="{{$chat->has_request()->supply_request}}" 
                                      data-payment_method="{{$chat->has_request()->payment_method}}"
                                      data-service_description="{{$chat->has_request()->service_description}}"
                                      data-amount="{{$chat->has_request()->amount }}">Edit Request</button>

                                    @else

                                      <button style="background-color: green; color: white; border-radius: 20px; border: 1px solid white; display: block; padding: 10px;" data-toggle="modal" data-target="#exampleModal" data-manufacturer_id="{{$chat->manufacturer->id}}" data-chat_id="{{$chat->id}}" data-product_id="{{$chat->product->id}}">Send Request</button>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <form  method="POST" action="{{ url('/admin/send-request') }}" enctype="multipart/form-data">
    @csrf
...
<div class="modal fade" id="editReqeust" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
    <form  method="POST" action="{{ url('/admin/send-update') }}" enctype="multipart/form-data">
    @csrf
      <div class="modal-header">
